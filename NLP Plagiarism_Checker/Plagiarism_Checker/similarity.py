...
class similar:
    ...
    def compare_documents(self, tf_idf_docs, tokens):
        compare = {}
        compare['query.txt'] = {}  # Initialize this dictionary

        for column in tf_idf_docs:
            if column == 'query.txt':
                continue  # skip comparing the query with itself

            query_length = math.sqrt(sum(tf_idf_docs['query.txt'].get(value, 0) ** 2 for value in tokens))
            doc_length = math.sqrt(sum(tf_idf_docs[column].get(value, 0) ** 2 for value in tokens))
            numerator = sum(tf_idf_docs['query.txt'].get(value, 0) * tf_idf_docs[column].get(value, 0) for value in tokens)

            if doc_length == 0 or query_length == 0:
                cosine_sim = 0
            else:
                cosine_sim = numerator / (doc_length * query_length)
